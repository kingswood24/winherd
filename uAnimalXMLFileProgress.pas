...
type
  TfmAnimalXMLIHMDataFile = class(TForm)
...
var
  fmAnimalXMLIHMDataFile: TfmAnimalXMLIHMDataFile;
...
procedure TfmAnimalXMLIHMDataFile.FormCreate(Sender: TObject);
begin
   FDOMDocument := nil;
...
class procedure TfmAnimalXMLIHMDataFile.ShowTheForm (ADataSet : TDataSet);
begin
   with TfmAnimalXMLIHMDataFile.Create(nil) do
      try
         FDataSet := ADataSet;
         ShowModal;
      finally
...
procedure TfmAnimalXMLIHMDataFile.FormActivate(Sender: TObject);
begin
   CreateXMLFile();
end;

procedure TfmAnimalXMLIHMDataFile.CreateXMLFile;
var
   D,
   M,
   Y : Word;
...
begin
   ...
   if not DirectoryExists(IncludeTrailingBackslash(FFileDirectory)+'Backup') then
      CreateDir(IncludeTrailingBackslash(FFileDirectory)+'Backup');
   ...
   pbAnimalXMLIHMDataFile.Properties.Min := 1;
   pbAnimalXMLIHMDataFile.Properties.Max := FDataSet.RecordCount;
   pbAnimalXMLIHMDataFile.Position := 1;
