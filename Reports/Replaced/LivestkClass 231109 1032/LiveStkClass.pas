...
uses
  ...
  RXCtrls, TB97, TB97Tlbr, QRExport;
...
uses Dairydata, GenTypesConst, uAccsReps, KRoutines;
...
begin
   ...
   with qLiveStkClass do
      begin
         ...
         qLiveStkClass.SQL.Clear;
         qLiveStkClass.SQL.Add('INSERT INTO tLiveStkClass (AnimalID, DOB, LactNum, PrimaryBreed, Breeding, Sex)');
         qLiveStkClass.SQL.Add('SELECT DISTINCT D.ID, D.DateOfBirth, D.LactNo, D.PrimaryBreed, Breeding, Sex   ');
...
//         pbCount.StepIt;
      ...
      with qLiveStkClass do
         begin
            SQL.Clear;
            SQL.Add('INSERT INTO tLiveStkClass (AnimalID, DOB, LactNum, PrimaryBreed, Breeding, Sex)');
            SQL.Add('SELECT DISTINCT D.ID, D.DateOfBirth, D.LactNo, D.PrimaryBreed, Breeding, Sex   ');
                  ...
                  if WinData.CheckEvents.Locate('AnimalID;EventType',VarArrayOf([tLiveStkClass.FieldByName('AnimalID').AsVariant,CPurchaseEvent]),[] ) then
                   ...
                   if ((tLiveStkClass.FieldByName('Include').AsBoolean = TRUE) AND (tLiveStkClass.FieldByName('LactNum').AsInteger > 0)) then
                      begin  //If still included and has a nonzero Lactation Number, then get Date Of First Calving - Note: The CheckEvents table is indexed on EventDate
                         if ( WinData.CheckEvents.Locate('AnimalID;EventType',VarArrayOf([tLiveStkClass.FieldByName('AnimalID').AsVariant,CCalvingEvent]),[] )) then
                            tLiveStkClass.FieldByName('FirstCalvingDate').AsDateTime := WinData.CheckEvents.FieldByName('EventDate').AsDateTime;
                      end;
                  Post;
                  Next;
                  pbCount.StepIt;
                ...
                while not (eof) do
                   begin
                      if not ( FieldByName('Include').AsBoolean ) then
                         ...
                         begin
                            if ( FieldByName('LactNum').AsInteger > 1 ) AND ( UPPERCASE(FieldByName('SEX').AsString) = 'FEMALE') then
                            ...
                            else if ( FieldByName('LactNum').AsInteger = 1 ) and ( FieldByName('FirstCalvingDate').AsDateTime <= CalcDate ) AND (UPPERCASE(FieldByName('SEX').AsString) = 'FEMALE') then
                               { Cow only where Calving date is before CalcDate (Current Month) }
                               begin
                                  BeefAnimal := (FieldByName('EligibleForPremium').AsBoolean);
                                  if ( BeefAnimal ) then
                                     { Suckler Cow }
                  ...
                  case ((nMonth+12-i) mod 12) of
                     ...
                     8 : begin
                           ...
                           if FileExists('C:\REPS_AUG.dat') then
                              DeleteFile('C:\REPS_AUG.dat');
                           //sl.SaveToFile('C:\REPS_AUG.dat');
                         end;
                     9 : begin
                            { September }
                            with LiveStkNums do
