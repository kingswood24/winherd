...
procedure TfFeedTypes.DBNavigatorBeforeAction(Sender: TObject;
  Button: TKNavigateBtn);
begin
   // save quantity to feed table
   if (Button = kwnbPost) then
      begin
         if ( WinData.FeedTypes.FieldByName('Category').AsInteger<=0) then
            begin
               MessageDlg('Category must be specified.',mtError,[mbOK],0);
               SysUtils.Abort;
            end;
      end;
end;
