      ...
      try
         ...
         if AddOrAmend = Add then
            begin
               ...
               bSet.Visible := not(cmboNatIDNum.Visible) and (DefaultCountry <> Ireland);
            end
         else
            begin
   ...
   if ( BullUsed.Value > '0' ) then
      begin
         ...
         if WinData.LookUpDamSire.Locate('ID',WinData.LastBull,[] ) then
            begin
               teSireName.Text := WinData.LookUpDamSire.FieldByName('Name').AsString;
               // Stet the Default Breed
               WinData.DefaultBreed := WinData.LookUpDamSire.FieldByName('PrimaryBreed').AsInteger;
               if ( WinData.TempCalving.State in dsEditModes ) then
                  begin
                     if WinData.AnimalFileByIDPrimaryBreed.AsInteger <> WinData.DeFaultBreed then
                        WinData.TempCalving.FieldByName('Breed').AsInteger := WinData.GetCrossBreed(WinData.DeFaultBreed)
                     else
                        WinData.TempCalving.FieldByName('Breed').AsInteger  := WinData.DeFaultBreed;
                  end;
            end;
      end
   else
...
begin
   ...
   if (DefaultCountry =Ireland) then
      lBirthType.Caption := 'ICBF Survey'
   else
...
begin
   ...
   if messagedlg('This event cannot be deleted once saved!'+#13#13+
                 '    Are all the details correct?',mtWarning,
...
{                           if WinData.TempCalving.RecordCount = 0 then
                              ...
                              if ( LookupComboValue( dblkcomboDamGroupListing ) > 0 ) and ( LookupComboValue( dblkcomboDamGroupListing ) <> DamFeedGroup ) then
                                 WinData.AddToGroup(DamID, dblkcomboDamGroupListing.KeyValue, TRUE, Date, '', DamTagNo )
                              else if ( dblkcomboDamGroupListing.Text = sRemoveFromGroup ) then
                                 begin
                                    GroupData := WinData.GroupUpdate.GetGroupData(DamFeedGroup,'Description');
                                    if MessageDlg(Format('Do you want to remove "%s" from feed group "%s"?',[WinData.AnimalFileByIDNatIDNum.AsString, GroupData[0]]),mtConfirmation,[mbYes,mbNo],0) = idYes then
                                       begin
                                          WinData.GroupUpdate.RemoveAnimalFromGrp(DamID, gtFeed);
...
begin
   ...
   WinData.LoadBtnImage(sbSaveCalvings.Glyph, cBtnImgSave);
   WinData.LoadBtnImage(sbDeleteCalving.Glyph, cBtnImgDelete);
   ...
   bSet.Visible := (DefaultCountry <> Ireland); // MK 08.05.07 - Program should know IE Prefix of Herd // shane 13/08/07
...
//         StatusBar.Visible := False;
   ...
   with TQuery.Create(nil) do
     try
        ...
        SQL.Add('SELECT GroupID FROM grps, grplinks');
   ...
   while NOT WinData.TempCalving.EOF do
      begin
         if ACountry = NIreland then
            ...
            begin
               if not WinData.CheckNatID(WinData.TempCalvingNatID.AsString, TempStr, False) then
                  begin
                     raise ErrorMsg.CreateFmt('Invalid National ID',[WinData.TempCalvingNatID.AsString]);
                  end;
            end;
...
begin
   ...
   if ( WinData.TempCalving.State in dsEditModes ) then
      begin
         if ( DefaultCountry = NIreland ) then
            begin
               DeadCalfNatIDValue := WinData.AnimalFileByIDNatIDNum.AsString;
               ...
               DeadCalfNatIDValue := DeadCalfNatIDValue + FormatDateTime('mm',Date) + FormatDateTime('yy',Date) + IntToStr(WinData.TempCalving.RecordCount+1);
               if Length(DeadCalfNatIDValue) > 16 then
                  begin
                  ...
                  begin
                     if CalfDead.EditValue = True then
                        begin
                           ...
                           try
                              WinData.TempCalvingNatID.AsString := DeadCalfNatIDValue;
                                    ...
                                    if e.Message <> 'National ID already entered' then
                                       ...
                                       begin
                                          MessageDlg(Format('The auto generated Calf National Id. No. "%s" is already in use.'+cCRLFx2+
                                                            'You must enter a value for the Calf National Id. No.',[DeadCalfNatIDValue]),mtWarning,[mbOK],0);
                        ...
                        begin
                           if (WinData.TempCalvingNatID.AsString = DeadCalfNatIDValue) then
                              WinData.TempCalvingNatID.Clear;
                        end;
                  end;
            end
         else if ( DefaultCountry = Ireland ) then
            begin
