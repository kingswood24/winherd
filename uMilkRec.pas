{
 ...
 12/08/10 [V4.0 R4.1] /MK Additional Feature - If Country is England then remove spaces and UK from Tag Number coming into the TempMilk.

 20/08/10 [V4.0 R4.5] /MK Additional Feature - If TempAnimals.NatIDNum has either UK or IE then strip UK or IE to match against TempMilk.

 23/08/10 [V4.0 R4.5] /MK Bug - CheckAnimals not working for anything other than England. I had to add a Begin/End in between
                                the IF Statement for FCountry.
                              - Strip Spaces from the Tag Number before it goes into the TempAnimals table so matching perfectly.

 28/08/10 [V4.0 R4.6] /MK Additional Feature - DoMilkRecording - Only Check NatID Against Animal No Where Country Is England.
                                             - ConvertDSMember - If Animal In C1 Line And C1 LiveFlag Is 0 Then Animal Live So Add To NMRAnimalList.

 30/08/10 [V4.0 R4.6] /MK Bug Fix - sbExecRecord - Daily BFat and Prot Not Being Saved If DaysInMilk > 305

 01/09/10 [V4.0 R4.7] /MK Bug Fix - DoMilkRecording - New Boolean, RecDateFound, will be set if this Milk Recording is
                                                      the same as last recording. Therefore the message "Milk Recording has not been saved"
                                                      will not appear.

 04/02/11 [V4.0 R8.0] /MK Bug Fix - uMilkRec - Added Line In CheckAnimals Where If AnimalCode or NatIDNum
                                               Does Not Exist On Milk File Then Inc NotMatched.

 23/02/11 [V4.0 R8.4] /MK Change - If English Herd Then Check For UKMilkRecorder In HerdSetup To Assign Caption
                                 - Changed Width of sbCISFileExport to fit QMMS.
                                 - Make sure that Milk Recorder in HerdSetup is selected before screen opens.

 18/10/11 [V5.0 R1.5] /MK Additional Feature - Added DSMEMBER.dat To DsMemberOpenDialog For Windows 7 DSMember Users.
                      /MK Change - ConvertDSMember - Do Not Run DSMember.exe As DatFile Already Set From OpenDialog For Windows7.
                                                   - Added Option To Set DatFile From OpenDialog For Windows7.

 10/07/12 [V5.0 R7.7] /MK Bug Fix - FormActivate - Made sure miUseICBFFormat was not visible if Country <> Ireland.
                                  - SetPositions - Removed checking for miUseICBFFormat.Visible here. Moved to OnActivate.

 22/10/12 [V5.1 R0.5] /MK Change - ConvertDSMember - Delete Exe and Dat files after file is read to converted.

 23/11/12 [V5.1 R1.2] /MK Bug Fix - ProcessMMRFile - Change MatchField and CurrAnimalSearch to AnimalID of live duplicate NatIDNum if one found.

 26/06/13 [V5.1 R7.5] /MK Change - ConvertDSMember - If DSMemberExeFileName is not on the C:\ drive then copy it to a temp directory.

 27/09/13 [V5.2 R1.6] /MK Additional Feature - Extract file from DSMember.exe with DynaZip instead of running
                                               exe file as exe file would not run with 64 bit.

 29/05/14 [V5.3 R1.6] /MK Change - FormActivate - Always match by NationalID if ICBF file.
                                 - SetPositions - If Country is not Ireland and MilkDiskParameter not selected then
                                                  bring up MilkDiskParameters screen.

 01/09/14 [V5.3 R5.7] /MK Change - CheckAnimals - English Herd - If NatID not found then match by AnimalNo.

 04/11/14 [V5.3 R8.8] /MK Change - GetOwnerData - Load File into string list first to save file with line breaks.

 19/04/16 [V5.5 R5.0] /MK Bug Fix - CompleteMMRRecording - Not setting IsSynchronized boolean on MilkDisk table.

 10/05/16 [V5.5 R5.5] /MK Bug Fix - bRecordClick - If WinData.ICBFMilkRec then is now FromDiskette option reinstated for Leslie Smith (5137).

 04/07/17 [V5.6 R9.1] /MK Bug Fix - FormCreate - sbCISFileExport should NOT be visible if OwnerFileUKMilkRecorder is DIY.

 21/02/19 [V5.8 R8.0] /MK Bug Fix - ProcessMMRFile - If difference found in lactations, report lact was being assigned as MyTable.LactNo - Willie O'Brien.

 23/08/19 [V5.9 R0.1] /MK Change - CreateAndShow - Pass in a variable AFileSaved that allows the system to know whether the file saved. 

 19/11/19 [V5.9 R1.3] /MK Change - SetPositions - Set the CountryCode variable to first character of selected country in Herd Settings.
                                                - Don't set country code based on MilkDiskLayout anymore as DSMember should be for all UK countries.
                                                - Added Wales caption change.
                                 - bRecordClick - Added Wales country code to DSMEMBER.DAT check and CallNMRStream.
                                 - DoMilkRecording - No Scotland rules anymore, all UK countries have the same format.
 ...
 20/11/19 [V5.9 R1.3] /MK Change - DoMilkRecording - No NI date change anymore, all UK countries have the same header date format.

 21/11/19 [V5.9 R1.3] /MK Change - Change Caption of screen to Milk Recording from Herd Settings - GL request.
 ...
 11/03/20 [V5.9 R2.8] /MK Change - cbDownloadFileClick - Look at Milk Recorder field in Owners table rather than preference - TGM request.
}
...
uses
  ...
  KRxSpeedButton, GenTypesConst, DynaZip, DynaLibU;
...
type
  ...
  private
    ...
    RecDateFound : Boolean;

    FUnzipFile: TUnzipFile;

    FUnZipArray : array[0..70] of char;
...
//    function  CheckFileExists(NameToCheck: String): Boolean;
    ...
    procedure CreateExportCaption;
    function LocateInHerdDupNatID(ANatIDNum : String) : Integer;
    function UnzipFileComplete(AFileName, ADestination: String): Boolean;
  public
     DuplicateIDArray : array of Integer;
     ThisAnimalArray : array of variant;
    { Public declarations }
     class procedure ImportICBFFile(AFileName: string;
        var AFileSaved : Boolean;
        const AMarkAttachmentAsRead : Boolean = True;
        const AFromICBFImportScreen : Boolean = False);
  end;

  procedure CreateAndShow(MMRType : Integer; var AFileSaved : Boolean); // Set a flag
...
const
    ...
    NMRFileName = 'dsmember.dat';
    NMRFileNameExe = 'dsmember.exe';
    NMRTempFileDir = 'C:\Kingswood Herd Management\TempNMRDir\';

var
  FFileName : string;
  FMarkAttachmentAsRead : Boolean;
  FFromICBFImportScreen : Boolean;
  FFileSaved : Boolean;

{$R *.DFM}
procedure CreateAndShow(MMRType : Integer; var AFileSaved : Boolean);
begin
...
//         TAnimals.FieldDefs := WinData.AnimalFileByID.FieldDefs;
         ...
         TAnimals.FieldDefs.Add('InHerd',ftBoolean);
...
procedure TfMilkRec.SetPositions;
   ...
   procedure EditMilkDiskLayout ( AID : Integer; ACountryStr : String );
   begin
      if ( WinData.MilkDiskLayout.Locate('ID',AID,[]) ) then
         try
            WinData.MilkDiskLayout.Edit;
            WinData.MilkDiskLayout.FieldByName('Country').AsString := ACountryStr;
            WinData.MilkDiskLayout.Post;
         except
            on e : Exception do
   ...
   begin
      WinData.qCountries.Close;
      WinData.qCountries.Open;
      if ( WinData.qCountries.Locate('ID',WinData.OwnerFile.FieldByName('Country').AsInteger,[]) ) then
         CountryCode := Copy(WinData.qCountries.FieldByName('Name').AsString,0,1)[1];
   end;

var
   MilkLayoutID : Integer;
   CountryCodeCheckCount : Integer;
begin
   ...
   try
      ...
      if WinData.Defaults.RecordCount > 0 then
         begin
            //   29/05/14 [V5.3 R1.6] /MK Change - If Country is not Ireland and MilkDiskParameter not selected then
            //                                     bring up MilkDiskParameters screen.
            ...
            if NOT WinData.MilkDiskLayout.Locate('ID',WinData.Defaults.FieldByName('MilkLayout').AsInteger,[] ) then
               ...
               begin
                  //   19/11/19 [V5.9 R1.3] /MK Change - Don't set country code based on MilkDiskLayout anymore as DSMember should be for all UK countries.
      ...
      //   19/11/19 [V5.9 R1.3] /MK Change - Set the CountryCode variable to first character of selected country in Herd Settings.
      {
      GetCountryCode;
      CountryCodeCheckCount := 0;
      while ( not(CountryCode in (['I','N','E','S','W'])) ) and ( CountryCodeCheckCount >= 0 ) do
         begin
            MessageDlg('Please select a Registered In from the Herd Settings screen.',mtError,[mbOK],0);
            uHerdSetup.ShowTheForm(False, 'Country');
            GetCountryCode;
            Inc(CountryCodeCheckCount);
         end;
      }

      //   10/07/12 [V5.0 R7.7] /MK Bug Fix - Removed checking for miUseICBFFormat.Visible here. Moved to OnActivate.
      if FCountry = Ireland then
         begin
            if ( WinData.ICBFMilkRec ) then
               Caption := 'Milk Recording - ICBF Recording'
            else
               Caption := 'Milk Recording - No Recording Type Selected';
         end
      else
         begin
            if ( Length(WinData.OwnerFileUKMilkRecorder.AsString) > 0 ) then
               Caption := 'Milk Recording - '+WinData.OwnerFileUKMilkRecorder.AsString+' Recording';
         end;
            ...
            begin
               if (Length( Trim( MyTable.FieldByName('NatIdNum').AsString)) > 0 ) then
                 begin
                    ...
                    //   01/09/14 [V5.3 R5.7] /MK Change - If NatID not found then match by AnimalNo.
                    else if ( TAnimals.Locate('AnimalNo',MyTable.FieldByName('AnimalCode').Value,[]) ) then
                       begin
                          ProcessAnimal;
                       end
                    else
...
begin
   ...
   with MyQuery do
      begin
         ...
         if WinData.ICBFMilkRec then
            begin
               ...
               //   10/05/16 [V5.5 R5.5] /MK Bug Fix - FromDiskette option reinstated for Leslie Smith (5137).
               if ( FromDiskette1.Checked ) then
                  begin
                     ICBFOpenDialog := TOpenDialog.Create(nil);
                     try
                        ICBFOpenDialog.DefaultExt := '.txt';
                        ICBFOpenDialog.Filter := 'TXT|*.txt';
                        ICBFOpenDialog.InitialDir := 'A:\';
                        ICBFOpenDialog.Execute;
                        if ( Length(ICBFOpenDialog.FileName) > 0 ) then
                           begin
                              WinData.MilkRecDir := ExtractDirFromPath(ICBFOpenDialog.FileName);
                              FFileName := ICBFOpenDialog.FileName;
                           end;
                     finally
                        FreeAndNil(ICBFOpenDialog);
                     end;
                  end;

               if ( FFileName = '' ) then
                  begin
                     ICBFFileRecord := GetICBFFile(WinData.MilkRecDir,HerdLookup.HerdOwnerData.HerdIdentity);
               ...
               if MilkFileName = '' then
                ...
                SQL.Clear;
                SQL.Add('INSERT INTO TempCows (ID, AnimalNo, NatIDNum, LactNo, InHerd, SortAnimalNo, SaleDeathDate) ');
                SQL.Add('SELECT A.ID, A.AnimalNo, A.NatIDNum, A.LactNo, A.InHerd, A.SortAnimalNo,');
                SQL.Add('       "'+FormatDateTime('mm/dd/yyyy', 0)+'" ');
      ...
      if WinData.ICBFMilkRec then
         ...
         begin
            //   19/11/19 [V5.9 R1.3] /MK Change - Added Wales country code to DSMEMBER.DAT check and CallNMRStream.
            if ( FCountry in [NIreland, England] ) then
               begin
   ...
   begin
      ...
      while NOT EOF(MyFile) do
         begin
            ...
            //   20/11/19 [V5.9 R1.3] /MK Change - No NI date change anymore, all UK countries have the same header date format.
            {
            ...
            //   19/11/19 [V5.9 R1.3] /MK Change - No Scotland rules anymore, all UK countries have the same format.
            {
...
// ******************************************* start of reporterrors
   ...
   begin
     ErrorCount := 0;
     if NOT NationalIDFound then
        begin
           MessageDLG('Error - No NationalID Number Has Been Found On The Kingswood Database',mtInformation,[mbOK],0);
           Inc(Errorcount);
        end;
     if NOT DateSet then
        begin
   ...
   try
      ...
      while NOT TAnimals.EOF do
         begin
            ...
            if ( FCountry = England ) then
               begin
                  //   12/08/10 [V4.0 R4.1] /MK Change - Check For Characters Other Than The UK.

                  NatIDNum := TAnimals.FieldByName('NatIDNum').Value;
                  NationalIDFound := True;

                  if ( Copy(NatIDNum, 1,2) = 'UK' ) or ( Copy(NatIDNum, 1,2) = cTagPreFix_IE ) then
                     NatIDNum := ( Copy(NatIDNum, 3,Length(NatIDNum) ))
                  else if ( Copy(NatIDNum, 1,2) = cTagPreFix_372) then
                     NatIDNum := ( Copy(NatIDNum, 4,Length(NatIDNum) ));

                  TAnimals.FieldByName('NatIDNum').Value := WinData.StripAllSpaces(NatIDNum);
      ...
      if CheckDates then
         begin
            ...
            if ( NOT GetLastRecording(False) ) then Exit;
                     ...
                     begin
                        ...
                        if (( Copy(DiskLine, 20, 2 ) <> '**' ) AND
                           ...
                           begin
                              ...
                              try
                                 MyTable.FieldByName('AnimalCode').Value := TempStr;
                                 //   28/08/10 [V4.0 R4.6] /MK Additional Feature - Only Check NatID Against Animal No Where Country Is England
                                 //   19/11/19 [V5.9 R1.3] /MK Change - Added Scotland and Wales to this check as NMR is now specified by Herd Settings screen.
                                 if ( FCountry in [NIreland, England] ) then
                                    begin
                                       if (NMRAnimalList <> nil) then
                                          begin
                                             ...
                                             if (Trim(TempStr) <> '') then
                                                begin
//                                                 12/08/10 [V4.0 R4.1] /MK Change - Strip Spaces From Tag Number from CIS File Tag Number.
                                                   MyTable.FieldByName('NatIDNum').Value := WinData.StripSpaces(TempStr);
                                                end;
                                          end;
                                    end;
                              except
                              ...
                              TempStr := WinData.StripSpaces(TempStr);
                              try
                                 //   19/11/19 [V5.9 R1.3] /MK Change - Added Scotland and Wales to this check for CumYield conversion.
                                 if ( FCountry in [NIreland, England] ) then
                                    MyTable.FieldByName('CumYield').Value := StrToFloat(TempStr)/10
                                 else
                              ...
                              //   19/11/19 [V5.9 R1.3] /MK Change - Added check for not England, Scotland or Wales for set of NoOfRecordings value. 
                              if ( FCountry = England ) then
                                 begin
                                    // Get the No of Tests
      ...
      try
              ...
              if NOT TidyUpDisk then
                 begin
                   ...
                   //   19/11/19 [V5.9 R1.3] /MK Change - Added Wales to processing of change of 305 yield data.
                   if ( FCountry = England ) then
                      begin
                         MyTable.First;
                         while not MyTable.Eof do
                                  ...
                                  begin
                                     ...
                                     if Do305YieldCalc then
                                        begin
                                           ...
                                           //  30/08/10 [V4.0 R4.6] /MK Bug Fix - Daily BFat and Prot Not Being Saved If DaysInMilk > 305
                   ...
                   {
                   TransQuery.First;
                   while ( not(TransQuery.Eof) ) do
                      begin
                         try
                            WinData.MilkDiskTrans.Append;
                            WinData.MilkDiskTrans.FieldByName('ID').AsInteger := TransQuery.FieldByName('ID').AsInteger;
                            WinData.MilkDiskTrans.FieldByName('MDTID').AsInteger := TransQuery.FieldByName('MDTID').AsInteger;
                            WinData.MilkDiskTrans.FieldByName('AnimalID').AsInteger := TransQuery.FieldByName('AnimalID').AsInteger;
                            WinData.MilkDiskTrans.FieldByName('AnimalCode').AsString := TransQuery.FieldByName('AnimalCode').AsString;
                            WinData.MilkDiskTrans.FieldByName('Lactno').AsInteger := TransQuery.FieldByName('Lactno').AsInteger;
                            WinData.MilkDiskTrans.FieldByName('HerdNumber').AsString := TransQuery.FieldByName('HerdNumber').AsString;
                            WinData.MilkDiskTrans.FieldByName('DateOfRecording').AsDateTime := TransQuery.FieldByName('DateOfRecording').AsDateTime;
                            WinData.MilkDiskTrans.FieldByName('DAilyYield').AsFloat := TransQuery.FieldByName('DAilyYield').AsFloat;
                            WinData.MilkDiskTrans.FieldByName('DailyBFatPerc').AsFloat := TransQuery.FieldByName('DailyBFatPerc').AsFloat;
                            WinData.MilkDiskTrans.FieldByName('DailyProtPerc').AsFloat := TransQuery.FieldByName('DailyProtPerc').AsFloat;
                            WinData.MilkDiskTrans.FieldByName('DailyLactPerc').AsFloat := TransQuery.FieldByName('DailyLactPerc').AsFloat;
                            WinData.MilkDiskTrans.FieldByName('SCC').AsInteger := TransQuery.FieldByName('SCC').AsInteger;
                            WinData.MilkDiskTrans.FieldByName('CumYield').AsFloat := TransQuery.FieldByName('CumYield').AsFloat;
                            WinData.MilkDiskTrans.FieldByName('CumBFatPerc').AsFloat := TransQuery.FieldByName('CumBFatPerc').AsFloat;
                            WinData.MilkDiskTrans.FieldByName('CumProtPerc').AsFloat := TransQuery.FieldByName('CumProtPerc').AsFloat;
                            WinData.MilkDiskTrans.FieldByName('CumLactPerc').AsFloat := TransQuery.FieldByName('CumLactPerc').AsFloat;
                            WinData.MilkDiskTrans.FieldByName('NoOfRecordings').AsInteger := TransQuery.FieldByName('NoOfRecordings').AsInteger;
                            WinData.MilkDiskTrans.FieldByName('DaysInMilk').AsInteger := TransQuery.FieldByName('DaysInMilk').AsInteger;
                            WinData.MilkDiskTrans.FieldByName('Yield305').AsFloat := TransQuery.FieldByName('Yield305').AsFloat;
                            WinData.MilkDiskTrans.FieldByName('BFat305Perc').AsFloat := TransQuery.FieldByName('BFat305Perc').AsFloat;
                            WinData.MilkDiskTrans.FieldByName('Prot305Perc').AsFloat := TransQuery.FieldByName('Prot305Perc').AsFloat;
                            WinData.MilkDiskTrans.FieldByName('Lact305Perc').AsFloat := TransQuery.FieldByName('Lact305Perc').AsFloat;
                            WinData.MilkDiskTrans.FieldByName('NMRRecording').AsBoolean := TransQuery.FieldByName('NMRRecording').AsBoolean;
...
begin
   ...
   // 04/07/17 [V5.6 R9.1] /MK Bug Fix - sbCISFileExport should NOT be visible if OwnerFileUKMilkRecorder is DIY.
   sbCISFileExport.Visible := ( (WinData.SystemRegisteredCountry <> Ireland) and
...
begin
   ...
   CheckRecordingOK;
   case MilkState of
      MFRead : if not RecDateFound then
                  if MessageDlg('Milk Recording has not been saved, do you want save?',mtConfirmation,[mbYes, mbNo],0) = idYes then
...
begin
   ...
   if FromDiskette1.Checked then
      begin
         DSMemberExeFileName := 'A:\'+NMRFileNameExe;
         if FileExists(DSMemberExeFileName) then
            begin
               CopyFile(PChar(DSMemberExeFileName), PChar(ExtractFilePath(ParamStr(0)) + NMRFileNameExe), False);
               DSMemberExeFileName := ExtractFilePath(ParamStr(0)) + NMRFileNameExe;
               OrigDSMemberLocation := DSMemberExeFileName;
               DSMemberDatFileName := ExtractFilePath(ParamStr(0))+'DSMEMBER.DAT';
            end
         else
            begin
         ...
         if DsMemberOpenDialog.Execute then
            begin
               if FileExists( DsMemberOpenDialog.FileName ) and (UPPERCASE(ExtractFileName(DsMemberOpenDialog.FileName)) = UPPERCASE( NMRFileNameExe )) then
                  begin
                     DSMemberExeFileName := DsMemberOpenDialog.FileName;
                     OrigDSMemberLocation := DSMemberExeFileName;
                     DSMemberDatFileName := ExtractFilePath(DsMemberOpenDialog.FileName)+'DSMEMBER.DAT';
                  end
               //   18/10/11 [V5.0 R1.5] /MK Change - Added Option To Set DatFile From OpenDialog For Windows7.
               else if (UPPERCASE(ExtractFileName(DsMemberOpenDialog.FileName)) = UPPERCASE( NMRFileName )) then
                  begin
                     DSMemberExeFileName := DsMemberOpenDialog.FileName;
                     OrigDSMemberLocation := DSMemberExeFileName;
                     DSMemberDatFileName := DsMemberOpenDialog.FileName;
                  end
               else
                  begin
                     MessageDlg('DsMember.exe file not found',mtError,[mbOK],0);
   ...
   if ( not(FileExists(DSMemberExeFileName)) ) then Exit;

   //   18/10/11 [V5.0 R1.5] /MK Change - Do Not Run DSMember.exe As DatFile Already Set From OpenDialog For Windows7.
   if ( not(FileExists(DSMemberDatFileName)) ) then
      begin
         //   26/06/13 [V5.1 R7.5] /MK Change - If DSMemberExeFileName is not on the C:\ drive then copy it to a temp directory,
         //                                     open it and then copy milk recording data from temp directory dat file.
         if ( UpperCase(Copy(DSMemberExeFileName,0,3)) <> 'C:\' ) or
            ( UpperCase(Copy(DSMemberExeFileName,0,3)) <> 'A:\' ) then
            begin
               if ( not(DirectoryExists(NMRTempFileDir)) ) then
                  CreateDir(NMRTempFileDir);
               CopyFile(PChar(DSMemberExeFileName),PChar(NMRTempFileDir+NMRFileNameExe),False);
               DSMemberExeFileName := NMRTempFileDir+NMRFileNameExe;
               DSMemberDatFileName := NMRTempFileDir+'DSMEMBER.DAT';
            end;
         //   27/09/13 [V5.2 R1.6] /MK Additional Feature - Extract file from DSMember.exe with DynaZip instead of running
         //                                                 exe file as exe file would not run with 64 bit.
         if ( not(UnzipFileComplete(DSMemberExeFileName,NMRTempFileDir)) ) then
            Exit;
         {
         if not (WinData.CallProg.DSMember(Handle, DSMemberExeFileName)) then
            Exit;
         }
      end;

   try
      if FileExists(DSMemberDatFileName) then
         begin
            {$I-}
            AssignFile(MyFile, DSMemberDatFileName);
   ...
   //   ReSet (MyFile);		{ open new milk recording file }
      ...
      try
         ...
         while (not eof (MyFile)) do
            begin
               readln (MyFile, p_buffer);
               if (Copy(p_buffer,1,2) = 'C1') and (Copy(p_buffer,16,1) = '0')then
               //  28/08/10 [V4.0 R4.6] /MK Additional Feature - If Animal In C1 Line And C1 LiveFlag Is 0 Then Animal Live So Add To NMRAnimalList
                  begin
                     // Store AnimalNo To StringList So We Can Reference This To DeadAnimalNo
                     TempValue := WinData.InsertZeros(copy(p_buffer,18,4), 5);
                      // store the animalno.
                     if copy(p_buffer,39,1) = '2' then
                        begin
                           NMRAnimalList.Values[ TempValue ] := copy(p_buffer,26,12);
                        end
                     else if copy(p_buffer,39,1) = '1' then
                        begin
                           readln (MyFile, p_buffer);
                           if (Copy(p_buffer,1,2) = 'C2') then
                              NMRAnimalList.Values[ TempValue ] := Copy(p_buffer,7,12);
                        end;
                  end;
         end;
   ...
   //                  NMRLactUpdate.Strings[NMRLactUpdate.Count-1] := Copy( NMRLactUpdate.Strings[NMRLactUpdate.Count-1], 1, Length(NMRLactUpdate.Strings[NMRLactUpdate.Count-1])-5);
      ...
      //   22/10/12 [V5.1 R0.5] /MK Change - Delete Exe and Dat files after file is read to converted.
      if ( FileExists(DSMemberDatFileName) ) then
         begin
            DeleteFile(DSMemberExeFileName,True);
            DeleteFile(DSMemberDatFileName,True);
         end;

      //   03/06/16 [V5.5 R5.8] /MK Bug Fix - When the change was made to take the extract the DSMember file, the original DSMember file was not deleted.
      if ( FileExists(OrigDSMemberLocation) ) then
         DeleteFile(OrigDSMemberLocation,True);
         ...
         begin
            ...
            if Copy(Buffer, 1, 3) = 'AID' then
               begin
                  CurrLact := True;
                  Parse(Buffer);
                  AnimalNo := StripLeadingZeros(Parsefields[6]);
                  CurrAnimalSearch := AnimalNo;
             ...
             //   23/11/12 [V5.1 R1.2] /MK Bug Fix - Change MatchField and CurrAnimalSearch to AnimalID of live duplicate NatIDNum if one found. 
             DupNatIDInHerdAID := LocateInHerdDupNatID(NatIDNum);
             if DupNatIDInHerdAID > 0 then
                begin
                   MatchField := 'ID';
                   CurrAnimalSearch := IntToStr(DupNatIDInHerdAID);
                end;
            ...
            if Copy(Buffer, 1, 3) = 'MMR' then
               begin
                  ...
                  MyTable.FieldByName('AnimalCode').AsString := AnimalNo;
                  MyTable.FieldByName('NatIDNum').AsString := NatIDNum;

                  if MatchField = 'NatIDNum' then // Locate with National Identity Number
                     CurrAnimalSearch := NatIDNum;

                  if TAnimals.Locate(MatchField, CurrAnimalSearch, [] ) then
                     begin
                     ...
                     begin
                        if MatchField = 'AnimalNo' then // Locate with National Identity Number
                           CurrAnimalSearch := MyTable.FieldByName('AnimalCode').AsString
                        else
                           CurrAnimalSearch := MyTable.FieldByName('NatIDNum').AsString;

                        if ( not tICBFRec.Locate(MatchField, CurrAnimalSearch, []) ) then
                           begin
                              tICBFRec.Append;
                        ...
                        begin
                           ...
                           if not NotMatched then
                              begin
                                 ...
                                 if MatchField = 'AnimalNo' then
                                    TempMilkDisk.FieldByName('AnimalCode').AsString := CurrAnimalSearch
                                 else if MatchField = 'NatIDNum' then
                                    TempMilkDisk.FieldByName('NatIDNum').AsString := CurrAnimalSearch
                                 else if MatchField = 'ID' then
                                    TempMilkDisk.FieldByName('NatIDNum').AsString := NatIDNum;
                              ...
                              begin // store mis-match.
                                 if not tICBFRec.Locate('AnimalID', TAnimals.FieldByName('ID').Value, []) then
                                    begin
                                       ...
                                       tICBFRec.FieldByName('NatIDNum').Value := TAnimals.FieldByName('NatIDNum').Value;
                                       tICBFRec.FieldByName('LactNo').Value := TAnimals.FieldByName('LactNo').AsInteger;
                                       //   21/02/19 [V5.8 R7.0] /MK Bug Fix - If difference found in lactations, report lact was being assigned as MyTable.LactNo - Willie O'Brien.
                                       tICBFRec.FieldByName('ICBFLactNo').Value := TempMilkDisk.FieldByName('LactNo').AsInteger;
                                       tICBFRec.FieldByName('UpdateType').Value := 0;
                                       tICBFRec.Post;
                                    end;
                                 TempMilkDisk.Cancel;
               ...
               begin
                  if ( AnimalIsFemale ) then
                     begin
                        ...
                        if TAnimals.Locate( MatchField, CurrAnimalSearch, [] ) then // make sure animal is found in TAnimals!
                           begin
...
var                                                 { Match With HerdNum.Value }
   Buffer : String;
   slMilkFile : TStringList;
begin
   GetMilkRecID;

   //   04/11/14 [V5.3 R8.8] /MK Change - Load File into string list first to save file with line breaks.
   slMilkFile := TStringList.Create();
   slMilkFile.LoadFromFile(MilkFileName);
   slMilkFile.SaveToFile(MilkFileName);
   FreeAndNil(slMilkFile);
   ...
   // check for Drying off event for curr lact.
{
   ...
   // Get the last recorded calving date of specified animal Id
   CalvingDate := WinData.EventDataHelper.GetLastEventDate(AID, LactNo, cCalvingEvent);
   // Only proceed to record drying off date if it is after the last recorded calving date
   if (CalvingDate > 0) and (EDate > CalvingDate) then
      begin
         // Only record a dryoff if there isn't already a dry off for specified LactNo
         if NOT(WinData.CheckEvents.Locate('AnimalID;AnimalLactNo;EventType',VarArrayOf([AID, LactNo, cDryOffEvent]),[])) then
            begin
               DryOffEvent := THealthEvent.Create('HRec');
               try
                  DryOffEvent.Append;
                  DryOffEvent.EventType    := TDryOff;
                  DryOffEvent.AnimalID     := AID;
                  DryOffEvent.AnimalLactNo := LactNo;
                  DryOffEvent.AnimalHerdID := HerdLookup.GetHerdID(AID);
                  DryOffEvent.EventDate    := EDate;
                  DryOffEvent.Post;
               finally
                  DryOffEvent.Free;
               end;
...
begin
   try
      try
                  ...
                  begin
                     if MilkDisk.Locate('AnimalID;LactNo',VarArrayOf([TempMilkDisk.FieldByName('AnimalID').Value,TempMilkDisk.FieldByName('LactNo').Value]),[] ) then
                        begin
                           ...
                           //   19/04/16 [V5.5 R5.0] /MK Bug Fix - Not setting IsSynchronized boolean on MilkDisk table.
                           try
                              MilkDisk.FieldByName('IsSynchronized').AsBoolean := False;
                           except
         ...
         FilePath := ExtractFilePath(MilkFileName);
         if ( not(FFromICBFImportScreen) ) then
            begin
               if ( FFileName <> '' ) and ( FMarkAttachmentAsRead ) then // rename attachment file.
                  TFmFileAttachmentImport.MarkAttachmentAsRead(FFileName)
               else if FileExists(MilkFileName) and ( FilePath = GetSpecialFolder(CSIDL_DESKTOP) ) then
...
begin
   ...
   //   10/07/12 [V5.0 R7.7] /MK Bug Fix - Made sure miUseICBFFormat was not visible if Country <> Ireland.
   miUseICBFFormat.Visible := ( WinData.UserDefaultHerdCountry = Ireland ) ;
   ICBFMatching1.Enabled := UseICBF;

   //   29/05/14 [V5.3 R1.6] /MK Change - Always match by NationalID if ICBF file.
   if ( UseICBF ) then
      begin
         WinData.ICBFMatchField := 'NatIDNum';
         AnimalNumber1.Checked := False;
...
begin
   //   19/11/19 [V5.9 R1.3] /MK Change - Added Wales to Update305FromNMRFile.
   if ( FCountry = England ) then
      begin
...
begin
   FFileName := AFileName;
   FMarkAttachmentAsRead := AMarkAttachmentAsRead;
   FFromICBFImportScreen := AFromICBFImportScreen;
   CreateAndShow(1,AFileSaved);
   AFileSaved := FFileSaved;
end;
...
begin
   ...
   //   11/03/20 [V5.9 R2.8] /MK Change - Look at Milk Recorder field in Owners table rather than preference - TGM request.
   if ( WinData.OwnerFileUKMilkRecorder.AsString = cUKMilkRecorder_NMR ) then
      OpenUrl(cNMRURL)
   else if ( WinData.OwnerFileUKMilkRecorder.AsString = cUKMilkRecorder_CIS ) then
      OpenUrl(cUnitedDairiesURL);
end;

procedure TfMilkRec.FormDestroy(Sender: TObject);
begin
   if ( NMRLactUpdate <> nil ) then
      FreeAndNil(NMRLactUpdate);
   if ( NMRAnimalList <> nil ) then
      FreeAndNil(NMRAnimalList);
   if FileExists('C:\NMRLactUpdate.dat') then
      DeleteFile('C:\NMRLactUpdate.dat');
end;

procedure TfMilkRec.CreateExportCaption;
begin
   sbCISFileExport.Caption := 'Create Milk Rec Company Export File';
   if ( Length(WinData.OwnerFileUKMilkRecorder.AsString) > 0 ) then
      sbCISFileExport.Caption := 'Create '+WinData.OwnerFileUKMilkRecorder.AsString+' Export File';
end;

function TfMilkRec.LocateInHerdDupNatID(ANatIDNum : String) : Integer;
begin
   ...
   with TQuery.Create(nil) do
      try
         ...
         SQL.Clear;
         SQL.Add('SELECT ID, NatIDNum, AnimalNo, LactNo SortAnimalNo, InHerd');
         SQL.Add('FROM '+TAnimals.TableName+'');
...
function TfMilkRec.UnzipFileComplete ( AFileName, ADestination : String ) : Boolean;
begin
   FillChar( FUnZipArray, SizeOf(FUnZipArray), #0); // Set Array contents to null
   StrPCopy( FUnZipArray, ADestination  + #0);
   FUnzipFile := TUnzipFile.Create(nil);
   with FUnzipFile do
      try
         lpszDestination.SetText(FUnZipArray);
         lpszZipFile.SetText(PChar(AFileName + #0));
         lpszFileSpec.SetText('*.*' + #0);
         Result := Execute(UNZIP_EXTRACT);
      finally
         FreeAndNil(FUnzipFile);
      end;
end;
