...
begin
   try
      ...
      if EventSource = esParlourLink then
         begin
            if MessageDlg('Do you want to review Parlour events?',mtConfirmation,[mbYes, mbNo],0) = idYes then
               ShowTheForm(TDBDataSet(EventTable), ErroneousItems, nil, LinkType )
         end
      else
         begin
      }
            ...
            while not EventTable.eof do
               begin
                  ...
                  if ( EventTable.FieldByName('EventType').AsInteger = CHealthEvent ) then
                     if ( WinData.CheckEventExists(EventTable.FieldByName('AnimalID').AsInteger,
                                                   CHealthEvent,EventTable.FieldByName('EventDate').AsDateTime) ) then EventTable.Next;
                  if ( EventTable.FieldByName('EventType').AsInteger = CDryOffEvent ) then
                     if ( WinData.CheckEventExists(EventTable.FieldByName('AnimalID').AsInteger,
                                                   CDryOffEvent,EventTable.FieldByName('EventDate').AsDateTime) ) then EventTable.Next;

                  if EventTable.FieldByName('EventType').AsInteger in ([CHealthEvent, CDryOffEvent]) then
                     begin
