{
  30/09/10 [V4.0 R4.9] /MK Change - Save new DueToCalve Report RadioGroups - rgShowAnimalNoOrNatID, rgDueCalveSortBy
                                  - Used SortOrderDefault for DueToCalve.

  29/11/10 [V4.0 R6.0] /MK Additional Feature - Save rgShowSolidsorValue From Best Cows In Herd.

  29/11/10 [V4.0 R6.0] /MK Change - Removed Check Procedure As Not Used

  18/10/11 [V5.0 R1.5] /MK Additional Feature - Added ShowCalfSex Property For Due To Calve Report.

  18/06/12 [V5.0 R7.2] /MK Additional Feature - Added IncSumColumn To HeatServicePD's Defaults.

  18/06/12 [V5.0 R7.2] /MK Additional Feature - Added IncServices To HeatServicePD's Defaults.

  01/07/14 [V5.3 R3.0] /MK Additional Feature - Add default for new recheck field in DueToPD report.

  11/08/15 [V5.4 R7.2] /MK Additional Feature - Add default for new IncludePostServiceHeat check in DueToCalve report.

  18/11/15 [V5.5 R0.8] /MK Additional Feature - New TDueDryOffDefaults property IncludeDueDryServedOrPDAfterHeat which is defaulted on - GL/Trevor Kellett request.

  21/03/18 [V5.7 R8.4] /MK Additional Feature - New TDuePregDiagDefaults property NotServedDaysToCalving which is defaulted to 50.

  30/07/18 [V5.8 R2.0] /MK Additional Feature - New TPurchaseSalesDefaults properties for IncludeAllPurchaseDetails and IncludeLastPurchaseDetails.

  11/11/20 [V5.9 R7.2] /MK Additional Feature - New TWeighingDefaults property for IncludeTargetWeights. 
}
...
//  29/11/10 [V4.0 R6.0] /MK Change - Removed As Not Used
//  procedure Check;
  ...
  private
    ...
    function GetIncludeAllPurchaseDetails: Boolean;
    function GetIncludeLastPurchaseDetails: Boolean;
    procedure SetIncludeAllPurchaseDetails(const Value: Boolean);
    procedure SetIncludeLastPurchaseDetails(const Value: Boolean);
  public
    constructor Create(AReportName : string);
    ...
    property IncludeLastPurchaseDetails : Boolean read GetIncludeLastPurchaseDetails write SetIncludeLastPurchaseDetails;
    property IncludeAllPurchaseDetails : Boolean read GetIncludeAllPurchaseDetails write SetIncludeAllPurchaseDetails;
  end;
  ...
  private
    ...
    function GetIncSumColumn: Integer;
    procedure SetIncSumColumn(const Value: Integer);
    function GetIncServices: Integer;
    procedure SetIncServices(const Value: Integer);
  public
     constructor Create(AReportName : string);
     ...
     property IncSumColumn : Integer read GetIncSumColumn write SetIncSumColumn;
     property IncServices : Integer read GetIncServices write SetIncServices;
  end;
  ...
  private
    ...
    function GetIncludeRecheckEvents: Boolean;
    procedure SetIncludeRecheckEvents(const Value: Boolean);
    function GetIncludeHeatAfterService: Boolean;
    procedure SetIncludeHeatAfterService(const Value: Boolean);
  public
     constructor Create(AReportName : string);
     ...
     property IncludeRecheckEvents : Boolean read GetIncludeRecheckEvents write SetIncludeRecheckEvents;
     property IncludeHeatAfterService : Boolean read GetIncludeHeatAfterService write SetIncludeHeatAfterService;
  ...
  private
    ...
    function GetNotServedDaysToCalving: Integer;
    procedure SetNotServedDaysToCalving(const Value: Integer);
  public
     constructor Create(AReportName : string);
     ...
     property NotServedDaysToCalving : Integer read GetNotServedDaysToCalving write SetNotServedDaysToCalving;
  end;

  { TDueDryOffDefaults }
  TDueDryOffDefaults = class(TActionReminderDefaults)
  private
    function GetIncludeDueDryServedOrPDAfterHeat: Boolean;
    procedure SetIncludeDueDryServedOrPDAfterHeat(const Value: Boolean);
  public
     constructor Create(AReportName : string);
     property IncludeDueDryServedOrPDAfterHeat : Boolean read GetIncludeDueDryServedOrPDAfterHeat write SetIncludeDueDryServedOrPDAfterHeat; 
  end;
  ...
  private
    ...
    function GetShowAnimalNoOrNatID: Integer;
    procedure SetShowAnimalNoOrNatID(const Value: Integer);
    function GetDueCalveSortOrder: Integer;
    procedure SetDueCalveSortOrder(const Value: Integer);
    function GetShowCalfSex: Integer;
    procedure SetShowCalfSex(const Value: Integer);
  public
     constructor Create(AReportName : string);
     property RestrictToPeriod : Boolean read GetRestrictToPeriod write SetRestrictToPeriod;
     property ShowAnimalNoOrNatID : Integer read GetShowAnimalNoOrNatID write SetShowAnimalNoOrNatID;
     property ShowCalfSex : Integer read GetShowCalfSex write SetShowCalfSex;
     property DueCalveSortOrder : Integer read GetDueCalveSortOrder write SetDueCalveSortOrder;
  end;
  ...
  private
    ...
    function GetIncludeTargetWeights: Boolean;
    procedure SetIncludeTargetWeights(const Value: Boolean);
  public
     constructor Create(AReportName : string);
     ...
     property PurchaseInterval : Integer read GetPurchaseInterval write SetPurchaseInterval;
     property IncludeTargetWeights : Boolean read GetIncludeTargetWeights write SetIncludeTargetWeights;
  end;
  ...
  private
    ...
    function GetShowSCCorIndex: Integer;
    procedure SetShowSCCorIndex(const Value: Integer);
    function GetShowSolidsorValue : Integer;
    procedure SetShowSolidsorValue(const Value: Integer);
  public
     constructor Create(AReportName : string);
     ...
     property ShowSCCorIndex : Integer read GetShowSCCorIndex write SetShowSCCorIndex;
     property ShowSolidsorValue : Integer read GetShowSolidsorValue write SetShowSolidsorValue;
  end;
...
function TPurchaseSalesDefaults.GetIncludeAllPurchaseDetails: Boolean;
begin
   if not Reg.ValueExists('IncludeAllPurchaseDetails') then
      IncludeAllPurchaseDetails := False;
   Result := Reg.ReadBool('IncludeAllPurchaseDetails');
end;

function TPurchaseSalesDefaults.GetIncludeLastPurchaseDetails: Boolean;
begin
   if not Reg.ValueExists('IncludeLastPurchaseDetails') then
      IncludeLastPurchaseDetails := True;
   Result := Reg.ReadBool('IncludeLastPurchaseDetails');
end;
...
procedure TPurchaseSalesDefaults.SetIncludeAllPurchaseDetails(
  const Value: Boolean);
begin
   Reg.WriteBool('IncludeAllPurchaseDetails', Value);
end;

procedure TPurchaseSalesDefaults.SetIncludeLastPurchaseDetails(
  const Value: Boolean);
begin
   Reg.WriteBool('IncludeLastPurchaseDetails', Value);
end;
...
function THeatsServicesPDs.GetIncServices: Integer;
begin
   if not Reg.ValueExists('IncServices') then
      IncServices := 0;
   Result := Reg.ReadInteger('IncServices');
end;

function THeatsServicesPDs.GetIncSumColumn: Integer;
begin
   if not Reg.ValueExists('IncSumColumn') then
      IncSumColumn := 0;
   Result := Reg.ReadInteger('IncSumColumn');
end;
...
procedure THeatsServicesPDs.SetIncServices(const Value: Integer);
begin
   if Value > -1 then
      Reg.WriteInteger('IncServices',Value);
end;

procedure THeatsServicesPDs.SetIncSumColumn(const Value: Integer);
begin
   if Value > -1 then
      Reg.WriteInteger('IncSumColumn',Value);
end;
...
function TActionReminderDefaults.GetIncludeHeatAfterService: Boolean;
begin
   if not Reg.ValueExists('IncludeHeatAfterService') then
      IncludeHeatAfterService := False;
   Result := Reg.ReadBool('IncludeHeatAfterService');
end;
...
function TActionReminderDefaults.GetIncludeRecheckEvents: Boolean;
begin
   if not Reg.ValueExists('IncludeRecheckEvents') then
      IncludeRecheckEvents := False;
   Result := Reg.ReadBool('IncludeRecheckEvents');
end;
...
procedure TActionReminderDefaults.SetIncludeHeatAfterService(
  const Value: Boolean);
begin
   Reg.WriteBool('IncludeHeatAfterService', Value);
end;
...
procedure TActionReminderDefaults.SetIncludeRecheckEvents(
  const Value: Boolean);
begin
   Reg.WriteBool('IncludeRecheckEvents', Value);
end;
...
function TDuePregDiagDefaults.GetNotServedDaysToCalving: Integer;
begin
   if not Reg.ValueExists('NotServedDaysToCalving') then
      NotServedDaysToCalving := 50;
   Result := Reg.ReadInteger('NotServedDaysToCalving');
end;
...
procedure TDuePregDiagDefaults.SetNotServedDaysToCalving(
  const Value: Integer);
begin
  Reg.WriteInteger('NotServedDaysToCalving', Value);
end;
...
function TDueDryOffDefaults.GetIncludeDueDryServedOrPDAfterHeat: Boolean;
begin
   if not Reg.ValueExists('IncludeDueDryServedOrPDAfterHeat') then
      IncludeDueDryServedOrPDAfterHeat := True;
   Result := Reg.ReadBool('IncludeDueDryServedOrPDAfterHeat');
end;

procedure TDueDryOffDefaults.SetIncludeDueDryServedOrPDAfterHeat(
  const Value: Boolean);
begin
   Reg.WriteBool('IncludeDueDryServedOrPDAfterHeat', Value);
end;
...
procedure TDueCalvingDefaults.SetShowAnimalNoOrNatID(const Value: Integer);
begin
   if Value > -1 then
      Reg.WriteInteger('ShowAnimalNoOrNatID', Value);
end;

function TDueCalvingDefaults.GetShowAnimalNoOrNatID: Integer;
begin
   if not Reg.ValueExists('ShowAnimalNoOrNatID') then
      ShowAnimalNoOrNatID := 0;
   Result := Reg.ReadInteger('ShowAnimalNoOrNatID');
end;

procedure TDueCalvingDefaults.SetDueCalveSortOrder(const Value: Integer);
begin
   if Value > -1 then
      Reg.WriteInteger('DueCalveSortOrder', Value);
end;

function TDueCalvingDefaults.GetDueCalveSortOrder: Integer;
begin
   if not Reg.ValueExists('DueCalveSortOrder') then
      DueCalveSortOrder := 0;
   Result := Reg.ReadInteger('DueCalveSortOrder');
end;

function TDueCalvingDefaults.GetShowCalfSex: Integer;
begin
   if not Reg.ValueExists('ShowCalfSex') then
      ShowCalfSex := 0;
   Result := Reg.ReadInteger('ShowCalfSex');
end;

procedure TDueCalvingDefaults.SetShowCalfSex(const Value: Integer);
begin
   if Value > -1 then
      Reg.WriteInteger('ShowCalfSex', Value);
end;
...
function TWeighingDefaults.GetIncludeTargetWeights: Boolean;
begin
   if not Reg.ValueExists('IncludeTargetWeights') then
      IncludeTargetWeights := True;
   Result := Reg.ReadBool('IncludeTargetWeights');
end;
...
procedure TWeighingDefaults.SetIncludeTargetWeights(const Value: Boolean);
begin
   Reg.WriteBool('IncludeTargetWeights', Value);
end;
...
function TBestCowsInHerdDefaults.GetShowSCCorIndex: Integer;
begin
   if not Reg.ValueExists('ShowSCCorIndex') then
      ShowSCCorIndex := 0;
   Result := Reg.ReadInteger('ShowSCCorIndex');
end;

function TBestCowsInHerdDefaults.GetShowSolidsorValue: Integer;
begin
   if not Reg.ValueExists('ShowSolidsorValue') then
      ShowSolidsorValue := 0;
   Result := Reg.ReadInteger('ShowSolidsorValue');
end;
...
procedure TBestCowsInHerdDefaults.SetShowSCCorIndex(const Value: Integer);
begin
   if Value > -1 then
      Reg.WriteInteger('ShowSCCorIndex', Value);
end;

procedure TBestCowsInHerdDefaults.SetShowSolidsorValue(
  const Value: Integer);
begin
   if Value > -1 then
      Reg.WriteInteger('ShowSolidsorValue', Value);
end;
