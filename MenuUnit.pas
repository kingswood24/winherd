...
uses
  ...
  uMessageScr, uModifyGridCols, KRoutines, uRemittanceConvert, uCalculateGridColSelect;
...
//    procedure qAnimalsAfterScroll(DataSet: TDataSet);
    ...
    procedure cxAnimalGridViewPrintAvgSalePrice(
      Sender: TcxDataSummaryItem; const AValue: Variant;
      AIsFooter: Boolean; var AText: String);
    procedure cxAnimalGridViewPrintAvgColdDeadWt(Sender: TcxDataSummaryItem;
  ...
  private
    ...
    FSelectedCalcGridCols : PStringArray;
...
begin
   ...
   actRefreshAnimals.Enabled := ( FSelectedHerdType <> htDairy );
   if ( FSelectedHerdType <> htDairy ) then
      if ( FSelectedHerdType = htBeef ) then
         begin
            SetLength(FSelectedCalcGridCols,2);
...
begin
   ...
   try
      ...
      if ( InArray(cxAnimalGridViewGrossMargin.Caption,FSelectedCalcGridCols) ) then
         begin
      ...
      if ( InArray(cxAnimalGridViewOverallGainPerDay.Caption,FSelectedCalcGridCols) ) then
         begin
            WinData.RefreshOverGainPerDay := False;
...
procedure TMenuForm.cxAnimalGridViewPrintAvgColdDeadWt(
  Sender: TcxDataSummaryItem; const AValue: Variant; AIsFooter: Boolean;
  var AText: String);
begin
   AText := '';
...
procedure TMenuForm.cxAnimalGridViewPrintAvgOverallGainPerDay(
  Sender: TcxDataSummaryItem; const AValue: Variant; AIsFooter: Boolean;
  var AText: String);
begin
